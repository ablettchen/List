...
public class List: NSObject {
    ...
    public var range: NSRange! {
        get {
            ...
            let lentgh = ((conf?.loadType == .new || conf?.loadType == .`none`) ? dataLengthMax : dataLengthDefault)
    ...
    @objc public func pull_loadNewData() -> Void {
        ...
        setStatus(.new)
        let length = ((self.conf?.loadType == .new || self.conf?.loadType == .`none`) ? dataLengthMax : dataLengthDefault)
    ...
    public func loadListData(_ listClosure: @escaping ListClosure) -> Void {
        ...
        if self.atList.conf?.loadStrategy == .auto {
            if self.atList.conf?.loadType == .`none` || self.atList.conf?.loadType == .more {
                ...
                let lentgh = ((self.atList.conf?.loadType == .new || self.atList.conf?.loadType == .`none`) ? dataLengthMax : dataLengthDefault)
